...
LONG_DESCRIPTION = """Faiss is a library for efficient similarity search and clustering of dense
...
INCLUDE_DIRS = [get_numpy_include(), FAISS_INCLUDE, FAISS_ROOT]
LIBRARY_DIRS = []
EXTRA_COMPILE_ARGS = []
EXTRA_LINK_ARGS = FAISS_LDFLAGS.split() if FAISS_LDFLAGS else []
SWIG_OPTS = [
    '-c++', '-Doverride=',
    '-I' + FAISS_INCLUDE,
    '-I' + FAISS_ROOT,
...
if sys.platform == 'win32':
    EXTRA_COMPILE_ARGS += [
        '/openmp', '/std:c++17', '/Zc:inline', '/wd4101', '/MD'
    ]
    EXTRA_LINK_ARGS += ['/OPT:ICF', '/OPT:REF']
    if not FAISS_LDFLAGS:
        EXTRA_LINK_ARGS += ['faiss.lib', 'openblas.lib']
    SWIG_OPTS += ['-DSWIGWIN']
elif sys.platform == 'linux':
    EXTRA_COMPILE_ARGS += [
        '-std=c++11', '-Wno-sign-compare', '-fopenmp',
        '-fdata-sections', '-ffunction-sections',
    ]
    EXTRA_LINK_ARGS += [
        '-fopenmp', '-lrt', '-s', '-Wl,--gc-sections',
    ]
    if not FAISS_LDFLAGS:
        EXTRA_LINK_ARGS += [
            '-L/usr/local/lib',
        ...
        if FAISS_ENABLE_GPU:
            EXTRA_LINK_ARGS += [
                ...
                '-lcudart_static', '-lculibos'
            ]
    SWIG_OPTS += ['-DSWIGWORDSIZE64']
elif sys.platform == 'darwin':
    EXTRA_COMPILE_ARGS += ['-std=c++11', '-Wno-sign-compare', '-Xpreprocessor', '-fopenmp']
    EXTRA_LINK_ARGS += ['-Xpreprocessor', '-fopenmp', '-dead_strip']
    if not FAISS_LDFLAGS:
        EXTRA_LINK_ARGS += ['-L/usr/local/lib', '-lfaiss', '-lomp', '-framework', 'Accelerate']
...
def create_extension(name_suffix, extra_compile_args, extra_link_args, swig_module_name):
    return Extension(
        f'faiss._swigfaiss{name_suffix}',
        sources=[
        ...
        swig_opts=SWIG_OPTS + ['-module', swig_module_name],
    )


ext_modules = [create_extension('', EXTRA_COMPILE_ARGS, EXTRA_LINK_ARGS, 'swigfaiss')]
...
if FAISS_OPT_LEVEL == 'avx2':
    ...
    ext_modules.append(create_extension('_avx2', extra_compile_args_avx2, extra_link_args_avx2, 'swigfaiss_avx2'))
